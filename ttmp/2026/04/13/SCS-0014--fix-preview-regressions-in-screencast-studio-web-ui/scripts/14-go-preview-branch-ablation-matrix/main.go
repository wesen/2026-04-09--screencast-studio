...
const (
	...
	previewFakesink    previewMode = "fakesink"
	previewJPEGDiscard previewMode = "jpeg-discard"
	previewRawCopy     previewMode = "raw-copy"
	previewCurrent     previewMode = "current"
	previewCheap       previewMode = "cheap"
)
...
func main() {
	...
	outputPath := envOr("OUTPUT_PATH", filepath.Join(os.TempDir(), "scs-preview-branch-ablation.mp4"))

	preview := previewProfile{Mode: previewNone}
	recorderEnabled := true
	switch scenario {
	case "recorder-only":
		preview = previewProfile{Mode: previewNone}
	case "preview-fakesink-plus-recorder":
		preview = currentPreviewProfile(region)
		preview.Mode = previewFakesink
	case "preview-jpeg-discard-plus-recorder":
		preview = currentPreviewProfile(region)
		preview.Mode = previewJPEGDiscard
	case "preview-raw-copy-plus-recorder":
		preview = currentPreviewProfile(region)
		preview.Mode = previewRawCopy
	case "preview-current-plus-recorder":
		preview = currentPreviewProfile(region)
	case "preview-cheap-plus-recorder":
		preview = cheapPreviewProfile(region)
	default:
...
func buildPipeline(displayName string, rootW, rootH int, region rect, fps int, preview previewProfile, recorderEnabled bool, outputPath string) (*gst.Pipeline, *combinedMetrics, *appsrcBridge, chan struct{}, chan *gst.Buffer, error) {
	...
	pipeline, err := gst.NewPipeline("preview-branch-ablation")
	if err != nil {
...
func buildPreviewBranch(preview previewProfile, metrics *combinedMetrics) ([]*gst.Element, error) {
	...
	elements := []*gst.Element{queue, videoscale, capsScale, videorate, capsRate}
	switch preview.Mode {
	case previewFakesink:
	...
	case previewJPEGDiscard, previewCurrent, previewCheap:
		...
		sink.SetCallbacks(&app.SinkCallbacks{NewSampleFunc: func(s *app.Sink) gst.FlowReturn {
			...
			if preview.Mode == previewJPEGDiscard {
				return gst.FlowOK
			}
		...
		elements = append(elements, jpegenc, sink.Element)
	case previewRawCopy:
		sink, err := app.NewAppSink()
		if err != nil {
			return nil, err
	...
	default:
		return nil, fmt.Errorf("unsupported preview mode %s", preview.Mode)
	}
	return elements, nil
}
...
func newAppsrcBridge(width, height, fps int, outputPath string) (*appsrcBridge, error) {
	pipeline, err := gst.NewPipeline("preview-branch-ablation-bridge")
	if err != nil {
		return nil, err
	}
...
func currentPreviewProfile(region rect) previewProfile {
	if region.W <= 1280 {
		return previewProfile{Mode: previewCurrent, Width: region.W, Height: region.H, FPS: 10, Quality: 80}
	}
	h := roundedScaledHeight(region.W, region.H, 1280)
	return previewProfile{Mode: previewCurrent, Width: 1280, Height: h, FPS: 10, Quality: 80}
}

func cheapPreviewProfile(region rect) previewProfile {
	if region.W <= 640 {
		return previewProfile{Mode: previewCheap, Width: region.W, Height: region.H, FPS: 5, Quality: 50}
	}
	h := roundedScaledHeight(region.W, region.H, 640)
	return previewProfile{Mode: previewCheap, Width: 640, Height: h, FPS: 5, Quality: 50}
}

func roundedScaledHeight(width, height, targetWidth int) int {
	if width <= 0 || height <= 0 || targetWidth <= 0 {
		return 0
	}
	v := float64(height) * (float64(targetWidth) / float64(width))
	if v < 0 {
		return int(v - 0.5)
	}
	return int(v + 0.5)
