...
cd "$REPO_ROOT"
gofmt -w "$SCRIPT_DIR/main.go" >/dev/null 2>&1 || true
...
run_case() {
  ...
  if [[ -f "$output_path" ]]; then
    ffprobe -hide_banner -loglevel error -show_entries format=duration,size:stream=codec_name,width,height,avg_frame_rate -of default=noprint_wrappers=1 "$output_path" > "$log_prefix.ffprobe.txt" || true
    echo "- output: $output_path" >> "$RUN_DIR/01-summary.md"
    sed 's/^/  /' "$log_prefix.ffprobe.txt" >> "$RUN_DIR/01-summary.md"
  fi
  echo "- counters:" >> "$RUN_DIR/01-summary.md"
  sed 's/^/  /' "$log_prefix.stdout.log" >> "$RUN_DIR/01-summary.md"
...
run_case preview-scale-first-current-plus-recorder preview-scale-first-current-plus-recorder
run_case preview-rate-first-current-plus-recorder preview-rate-first-current-plus-recorder
run_case preview-scale-first-constrained-plus-recorder preview-scale-first-constrained-plus-recorder
run_case preview-rate-first-constrained-plus-recorder preview-rate-first-constrained-plus-recorder
