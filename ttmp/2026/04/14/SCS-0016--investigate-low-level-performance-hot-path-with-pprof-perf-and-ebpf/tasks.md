...
- [x] Write down the exact high-signal repro that all low-level tools should target
- [x] Define what counts as a successful profile capture for pprof, perf, and eBPF
...
## Phase 1.5: small-graph hosting ladder

- [x] Write a detailed stage-by-stage debugging plan for the small-graph hosting ladder
- [x] Add ticket-local Go manual, Python manual, and `gst-launch` stage-ladder controls under `scripts/29-*` through `scripts/32-*`
- [x] Run the first full 6-stage ladder matrix across `capture`, `convert`, `rate-caps`, `encode`, `parse`, and `mux-file`
- [x] Identify the first strong divergence point between Go and the cooler Python / `gst-launch` controls
- [x] Decide whether the next target should be early raw-video stages, the encoder boundary, or the mux/output tail
...
- [x] Summarize whether the dominant hot path is in Go, CGO, GStreamer, libc, syscalls, or scheduler behavior

## Phase 3: eBPF only if still needed

- [x] Decide which eBPF questions remain unanswered after pprof/perf
...
- [x] Produce a lower-level findings report with concrete evidence and caveats
- [x] Connect the lower-level findings back to the SCS-0015 browser-path hypothesis
- [x] Recommend the next code-change target based on profiler evidence, not speculation
- [x] Validate the ticket with `docmgr doctor --ticket SCS-0016 --stale-after 30`
